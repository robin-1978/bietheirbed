...
def build_system_prompt(
    ...
    parts.extend([
        ...
        "- When the user mentions personal information (name, location, preferences, work, habits, etc.), use the `memory` tool with action `store` to save it for future conversations.",
        "- Before answering location-specific or preference-specific questions, use `memory` with action `retrieve` or `search` to check if you have relevant user information.",
        "- Examples: user says 'I live in Shanghai' → memory.store(key='location', value='Shanghai', category='location'); user says '我叫小明' → memory.store(key='name', value='小明', category='identity')",
        "",
        "## Output Format",
